...
export default function PlainningStudent() {
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await api.get(`downloadClassSchedule/${user.class?._id}`, { responseType: 'blob' }); // Request file as blob
      if (response.status === 401) {
        ...
        downloadLinkRef.current.download = `Emploie_du_temps_${user.class?.className}.pdf`; // Set the download attribute to specify filename
              ...
              <div className="bg-white shadow-md rounded p-6 flex flex-col md:flex-row items-center md:justify-between">
                <div className="flex items-center space-x-4 mb-4 md:mb-0">
                  <div className="rounded-full bg-gray-300 w-16 h-16 flex items-center justify-center text-xl font-bold text-gray-700">
                    {user.class?.className.split(' ').map(word => word.charAt(0)).join('')}
                  </div>
                  <div>
